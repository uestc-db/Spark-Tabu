...
class Combine extends Serializable
{
  ...
  def get_pairs(recs:Array[(Int,(Double,Double,Double,Double))]):Map[Int,Int]=                          //将分区分为两两构成的对
  {
  ...
  def get_pairs_divide(recs:Array[(Int,(Double,Double,Double,Double))]):Map[Int,Int]=
  {
    ...
    var diff_pairs:Array[(Int,(Double,Array[(Int,Int)]))]=Array()                  //单独的矩形id，距离，组队的矩形
    var single=(-1)
    if(recs.length%2==0)
    ...
    else
    {
      ...
      var diff_pairs_sort = diff_pairs.sortWith{(a,b) => (a._2._1 < b._2._1)}                 //按照距离排序 
    ...
    if(single!=(-1))
      key_id+=(single->(return_pairs.length/2+1))
  ...
  def get_pairs_even(recs:Array[(Int,(Double,Double,Double,Double))]):(Double,Array[(Int,Int)])=               //当有偶数个分区时
  {
    val Maxnum = 20
  ...
  def rec_bound(id_point_list:Array[(Int,Record)],f_orders:Map[String,Order], e_orders:Map[String,Order_2],points_set:Map[String,Point_id]):(Double,Double,Double,Double)=                       //计算一个solution的MBR
  {
    var up=(-90.0)
    var down=90.0
    var left=180.0
