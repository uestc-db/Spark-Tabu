...
object Logistics extends java.io.Serializable
{
  ...
  def main(args: Array[String])
  {
    ...
    var Max_iter:Int = 0
	val total_courier:Int = args(6).toInt
    var Tabu_list_length=args(1).toInt
    var part_num=args(2).toInt
    var data=util_define.read_data()
	var points_set=data._1.union(data._2.union(data._3)).toMap            //for dist()
	var site_points=data._1.toMap                                   //for get_flist() and get_elist()
    var shop_points=data._3.toMap                                    //for get_elist()
    ...
    val records_all:Array[(Int,Record)]=util_define.readRecord("hdfs://master:9000/user/liuyi/logistics/data/first.csv")
    ...
    var tree_height = rtree_patitioner.tree_height
    ...
    var region_num:Int = part_num
    while(combine_iter >= 0)
    {
        Max_iter = 16/Math.sqrt(part_num).toInt - tree_height + region_num
		if (Max_iter <0) Max_iter=3
        println("tree_height: "+tree_height+"region_num: "+region_num+"iter_num: "+Max_iter)
