class Path(Courier_id:String) extends Serializable
{
	...
	def cost(f_orders:Map[String,Order],e_orders:Map[String,Order_2],penal:Double,E_list:Map[String,String],points:Map[String,Point_id],mode:Int):(Double,Array[Int],Array[String],String)=                       //计算当前path的cost  
    {
    	var time_penalty=0
    	var penalsum=0
    	var now_deptime=0
    	...
    	val first=orders(0)                                      //path中的第一个订单
    	var last_spot=" "
    	...
    	while(it.hasNext)
    	{
            ...
            if(tmp.Oid.charAt(0)=='E')                             //O2O包裹
            {
            	...
            	if(check.contains(tmp.Oid))
            	{
            		...
            		now_deptime=now_arrtime+util.serv(read_order.num_ord)
            		last_spot=now_spot
            		if(now_deptime>read_order.time2)                                        //超时惩罚
            		{
            	...
            	else
            	{
            		...
            		{
            			now_deptime=now_arrtime
            			time_penalty=time_penalty+5*(now_arrtime-read_order.time1)          //迟到惩罚
            		}
            		now_load+=read_order.num_ord
            		if(now_load>140)
            			penalsum+=now_load-140
    ...
    def index2(id:String)=                              //返回在当前path中Oid=id的记录的下标
    {
    ...
    def delete_Record(id:String)=                          //删除当前path中Oid=id的记录
    {
    ...
    def insert_Record(id:String,index:Int):Array[Record]=                        //在当前path的index位置插入id订单
    {
    ...
    def dist_only(rorder:String,f_orders:Map[String,Order],e_orders:Map[String,Order_2],points:Map[String,Point_id],E_list:Map[String,String]):Int=                               //订单order的取货和送货4段路程的总时间
    {
    	var time_penalty=0 
    	var now_deptime=0
    	...
    	if (orders(0).Oid.charAt(0)=='F') 
    		last_spot=f_orders(orders(0).Oid).fInd     //第一个订单的起始位置
    	else
    ...
    def arc_length(f_orders:Map[String,Order],e_orders:Map[String,Order_2],penal:Double,E_list:Map[String,String],points:Map[String,Point_id]):(Int,Array[Int])=                       //计算当前path的cost  
    {
